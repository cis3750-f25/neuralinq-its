...
def update_metrics(student_data, student_id, question_id, skill, is_correct, time_spent, hints_used=0):
...
def get_question():
    ...
    # Pick a skill to teach based on performance metrics
    ...
    if not skill_questions:
        return jsonify({"error": f"No questions found for skill: {skill_to_teach}"}), 500

    question_to_send = random.choice(skill_questions)
...
def submit_answer():
    ...
    update_metrics(student_data, student_id, question_id, skill, is_correct, time_spent, hints_used)
...
def add_skill():
    ...
    normalized_skill = raw_skill.strip().lower().replace(' ', '_')
